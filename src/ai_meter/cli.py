"""Command line interface for AIMeter."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aimeter")
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "budget":
            store = SQLiteStore(Path(args.database))
            try:
                from ai_meter.core import evaluate_budget
        ...
        if args.command == "schema":
            ...
            print_json({"status": "exported", "output": str(output)})
            return 0
    except (AIMeterError, KeyError, OSError, json.JSONDecodeError, yaml.YAMLError) as exc:
        print(f"aimeter: error: {exc}", file=sys.stderr)
        return 2
...
def deprecated_app() -> None:
    message = "openaimeter has been renamed to aimeter and will be removed in a future release."
    warnings.warn(message, DeprecationWarning, stacklevel=2)
    print(f"openaimeter: warning: {message}", file=sys.stderr)
    raise SystemExit(run())
